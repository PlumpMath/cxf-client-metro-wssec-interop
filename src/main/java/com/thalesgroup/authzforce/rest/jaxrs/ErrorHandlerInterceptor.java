...
/**
 * 
 * CXF Interceptor for outbound faults that handles internal server errors. In particular, it remove sensitive info and maps it to Bad Request status code when it is actually a bad request.
 * Based on the CXF CustomOutFaultInterceptor example. 
 * @see <a href="http://svn.apache.org/repos/asf/cxf/trunk/systests/jaxrs/src/test/java/org/apache/cxf/systest/jaxrs/CustomOutFaultInterceptor.java">org.apache.cxf.systest.jaxrs.CustomOutFaultInterceptor</a>
 */
public class ErrorHandlerInterceptor extends AbstractPhaseInterceptor<Message>
{
	...
	static
	{
		...
		} catch (JAXBException e)
		{
			throw new RuntimeException(
					"Failed to initialize Authorization API schema's JAXB context for marshalling Error elements", e);
		}
	}
	...
	public void handleMessage(Message message) throws Fault
	{
		...
		if (ex != null)
		{
			...
			final HttpServletResponse response = (HttpServletResponse) message.getExchange().getInMessage()
			...
			try
			{
				...
				marshaller.marshal(errorEntity, response.getOutputStream());
